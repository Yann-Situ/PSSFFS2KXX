...
func _ready():
	...
	sprite_material = ball.get_node("Sprite").material
...
func set_selection_color(col):
	...
	sprite_material.set_shader_param("contour_color", col)

func toggle_selection(b):
	if b :
		set_selection_color(selection_color)
		#sprite_selection.visible = true
		sprite_material.set_shader_param("activated", true)
	else :
		sprite_material.set_shader_param("activated", false)
