...
@export var tolerance_jump_floor : float = 9*frame_time_ms #s
...
var is_onfloor = false # from values of Character.gd
var is_onwall = false # from values of Character.gd
var is_moving_fast = false # from values of Character.gd
...
# Delays and states memory # handle by Character.gd
#var last_onfloor = 0
var last_frame_onair = false
#var last_onwall = 0
#var last_jump = 0
#var last_walljump = 0
var last_wall_normal_direction = 0 # handle by Character.gd
...
func update_vars(delta):
	...
	Character.ActionHandler.update_space_state()
	Character.ActionHandler.update_basket()
	...
	#can_aim = $CanShootTimer.is_stopped() and has_ball and held_ball != null
	can_shoot = has_ball and held_ball != null
	can_dunkjump = can_jump and Character.ActionHandler.can_dunkjump()
	can_dunk = $CanDunkTimer.is_stopped() and \
		(is_dunkjumping or (not is_onfloor and dunk_p)) and \
		Character.ActionHandler.can_dunk()
	can_stand = Character.ActionHandler.can_stand()
	...
	is_halfturning = (is_halfturning or dir_sprite*direction_p == -1) and \
		is_onfloor and direction_p != 0 and not is_shooting # handle by player actions
	is_landing = is_onfloor and not is_onwall and (is_landing or \
		(last_frame_onair and \
			last_onair_velocity_y > Character.landing_velocity_thresh)) and \
		not is_halfturning# stop also handled by animation
	...
	#is_aiming = is_aiming and has_ball and held_ball != null and not is_dunkjumping
	#is_shooting handle by shoot animation+Character.gd
